...
try {
  ...
  check('both lens cards wait in Speak', (await lPage.locator('.lens-card').count()) === 2)
  await lPage.locator('.sign-lens .lens-begin').click()
  await lPage.waitForTimeout(300)
  check('no key points at Kit', ((await lPage.textContent('.sign-lens .lens-fail')) ?? '').includes('Kit'))

  await lPage.goto(`${BASE}/?lens=ok#speak`)
  await lPage.waitForTimeout(500)
  await lPage.locator('.sign-lens .lens-begin').click()
  await lPage.locator('.sign-lens input[type="file"]').setInputFiles({ name: 'sign.jpg', mimeType: 'image/jpeg', buffer: tinyJpeg })
  await lPage.waitForTimeout(900)
  check('the sign is decoded', (await lPage.locator('.sign-lens .lens-result').count()) === 1)
  const firstBlock = await lPage.locator('.sign-lens .lens-result > *').first().getAttribute('class')
  ...
  await lPage.locator('.sign-lens input[type="file"]').setInputFiles({ name: 'sign.jpg', mimeType: 'image/jpeg', buffer: tinyJpeg })
  await lPage.waitForTimeout(900)
  check('offline gets a calm face', ((await lPage.textContent('.sign-lens .lens-fail')) ?? '').includes('sky'))
  await shot(lPage, 'sign-lens')
  await lCtx.close()

  /* ---- 15b. The menu lens: dinner through the same eye ---- */
  const mCtx = await browser.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 2, isMobile: true, hasTouch: true })
  const mPage = await mCtx.newPage()

  await mPage.goto(`${BASE}/#speak`)
  await mPage.waitForTimeout(500)
  await mPage.locator('.menu-lens .lens-begin').click()
  await mPage.waitForTimeout(300)
  check('menu with no key points at Kit', ((await mPage.textContent('.menu-lens .lens-fail')) ?? '').includes('Kit'))

  await mPage.goto(`${BASE}/?menu=ok#speak`)
  await mPage.waitForTimeout(500)
  await mPage.locator('.menu-lens .lens-begin').click()
  await mPage.locator('.menu-lens input[type="file"]').setInputFiles({ name: 'menu.jpg', mimeType: 'image/jpeg', buffer: tinyJpeg })
  await mPage.waitForTimeout(900)
  check('the menu is read', (await mPage.locator('.menu-lens .lens-result').count()) === 1)
  const mFirst = await mPage.locator('.menu-lens .lens-result > *').first().getAttribute('class')
  check('menu warnings come first', (mFirst ?? '').includes('lens-warning'), mFirst ?? '')
  check('every dish gets a row', (await mPage.locator('.menu-dish').count()) === 5)
  const avoidText = (await mPage.locator('.menu-dish.avoid').textContent()) ?? ''
  check('the kiwi dish is struck in vermillion', avoidText.includes('キウイ'), avoidText)
  check('kid picks are marked', (await mPage.locator('.menu-kid').count()) === 3)
  check('the order line is drafted', ((await mPage.textContent('.menu-lens .lens-result')) ?? '').includes('これをください'))
  await shot(mPage, 'menu-lens')

  await mPage.goto(`${BASE}/?menu=offline#speak`)
  await mPage.waitForTimeout(500)
  await mPage.locator('.menu-lens .lens-begin').click()
  await mPage.locator('.menu-lens input[type="file"]').setInputFiles({ name: 'menu.jpg', mimeType: 'image/jpeg', buffer: tinyJpeg })
  await mPage.waitForTimeout(900)
  ...
  // the invite path: a synced family that just minted a code must SHOW it (v4.0.1 field fix)
  await ikPage.goto(`${BASE}/?ink=invited#kit`)
  await ikPage.waitForTimeout(500)
  check('a synced family can still show a fresh code', ((await ikPage.textContent('.ink-code', { timeout: 3000 }).catch(() => null)) ?? '').includes('NARA-07'))
