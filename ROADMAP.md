...
- **Today's Scroll** ✅ *(v1 built 2026-07-13 as the **Kairanban** 回覧板 — each phone's journal page publishes
  to the family, database-enforced read-only for everyone else, ink-hearts, photo thumbs in Treasures and on
  day pages; auto-binding photos to stops remains the v2 dream)* `L · backend` — Photos from all four phones
  auto-bind to the stop where they were taken and assemble each evening into a shared illustrated diary page.
...
| **v4.2** 📖 | 回覧板 *Kairanban* | The family's pages | **Shipped (merged 2026-07-14 JST, PR #26; tagged v4.2.0 + Release live; live E2E 13/13 incl. the vandal-refusal proof):** the journal becomes the family feed — day-entries auto-publish while the ink is on, database-enforced read-only for every phone but the writer's, photo thumbs (≤640px, cap 4) riding in the rows, one ink-heart per person per page, the Treasures dot, IndexedDB offline cache. Plus **test the brush** (the AI-key ping, born from the first field failure) and the security sweep's join-throttle + RPC-surface lock. Suite 144/144 |
| **v4.1** 🦊 | 道連れ *Michizure* | Companions on the road | **Shipped (merged 2026-07-13, PR #24; tagged v4.1.0 + Release live; built overnight, 130/130 checks):** **Menu Lens** (dinner through the #17 key: dishes in English, allergy flags with kid-plain whys, 子 kid picks, the polite order line) + **kiwi** on the allergy card + the **generative kamon** (the crest the trip paints — travelers' animals as folds, a petal per loved moment, sealing Family Ink and Treasures) + **Sumi v1** (the ink spirit above the tab bar: breathes, sleeps by the solar clock, joy-splashes on loved moments, Nara antlers, pettable). Deferred from the six-feature talk: Phrase Dojo, generative soundscape, opening cinematic |
| **v4.3** 🧮 | 算盤 *Soroban* | The abacus | **Built (2026-07-14, trip day three, branch `claude/pace-forecast-spend-api-7ave39`; awaiting merge):** the **Spend pace** in Kit → Spend — set the trip's cash budget once, log what a day cost, and an offline forecast divides what's left across the days that remain (today's share, each day ahead); the **pace coach** asks Claude to weigh the pouch against the itinerary still ahead (#17 key, `?pace=` fixtures). Riding along: **both lenses + the key test switch to Sonnet** (`claude-sonnet-5`, DECISIONS #28) for faster answers at the counter. Suite 156/156 |
