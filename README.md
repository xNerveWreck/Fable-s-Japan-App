...
**As of 2026-07-11 (late) — v3.5.0 built on `claude/real-itinerary`, awaiting the
owner's merge** (tag + Release follow the merge; last shipped:
[v3.2.0](https://github.com/xNerveWreck/Fable-s-Japan-App/releases/tag/v3.2.0)).
The branch carries the two big ones: **the real itinerary** — the fictional
14-day dream trip is fully replaced by the booked Jul 12–23 plan (12 dated day
cards, Gion Matsuri week, Puroland, USJ, Katsuōji; 62 haiku re-engraved; route
map, quests, Denshadex, and Kit all follow) — and the **Sign & Etiquette
Decoder** (photograph any Japanese sign from the Speak tab; BYO Anthropic key
pasted in Kit → Settings, stored on-device only). 94-check suite green.
`main` still serves v3.2.0 until the merge; Pages auto-deploys on merge.
...
| | |
|---|---|
| ⛩️ **Journey** | The real 12-day itinerary as a living record — every card dated (Sun · Jul 12 → Thu · Jul 23), from **The Leap** (the flight day the date line eats) through Gion Matsuri week to the plane that lands before it takes off. Every day has a theme, a painted city vignette, a timeline of stops, honest family pacing in July heat, a rain plan, a **reservations pocket** for confirmation codes, and 🦊 *For the kids* tips on nearly every activity. Mark each stop **Did it / Loved it / Skipped** — loved moments collect into a treasures list, and each one is quietly **engraved with its own 5-7-5** (sixty-two haiku ship in the bundle, one per stop); the *短冊 unroll* button opens the **tanzaku scroll** — every loved poem on one washi strip, in trip order, sealed with a hanko. Built for the flight home. The home screen carries the countdown (then live *Up next today* during the trip), **The Road** — the route as one brushstroke with a dashed arc home, where a red hanko stamp lands on each completed city — a rotating phrase of the day, and the **stamp journal**: twelve eki-stamp badges earned day by day. From any day page, flip through the trip like pages of a scroll — chevrons in the header, painted next/previous cards at the foot. The bullet-train day opens the **Fuji Window** (below); the Nara day runs the **Deer Dojo**, a practice round of shika-senbei protocol that becomes a live exchange log with ranks; and every city hides **Side Quests** — find-it micro-hunts that unlock on arrival, where two finds paint a secret detail into that city's vignette forever. |
