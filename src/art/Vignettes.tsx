 ...
 * snow, lantern gold.
 *
 * Since the living-vignettes work each scene is layered into depth planes
 * (vg-far / vg-mid / vg-near) that tilt with the phone or drift on their
 * own, plus classed inhabitants (heron, gondola, deer bow, neon…) whose
 * motion lives entirely in CSS — see the vignettes section of screens.css.
 */
...
function TokyoVignette() {
  return (
    <Stage label="Tokyo skyline with Tokyo Tower at dusk">
      ...
      {/* near skyline, windows lit one at a time after dark */}
      <g className="vg-mid">
        ...
        <g fill="var(--art-lantern)" opacity="0.8">
          <rect className="vg-win" x="60" y="74" width="4" height="4" />
          <rect className="vg-win" x="68" y="86" width="4" height="4" />
          <rect className="vg-win" x="296" y="74" width="4" height="4" />
          <rect className="vg-win" x="330" y="90" width="4" height="4" />
          <rect className="vg-win" x="26" y="84" width="4" height="4" />
        </g>
      </g>
      {/* Tokyo Tower */}
      <g className="vg-near">
      ...
      {/* sketch birds, drifting the width of the sky */}
      <g className="vg-birds" stroke="var(--art-silhouette)" strokeWidth="1.8" fill="none" strokeLinecap="round">
  ...
  return (
    <Stage label="A tunnel of vermillion torii gates">
      {/* the heron: parked offscreen left; the CSS keyframe flies it across,
          painted before the gates so it passes behind their crossbeams */}
      ...
      {/* receding torii tunnel, deepest scene of the six */}
      <g className="vg-far">{GATES.slice(3).map((g, i) => torii(g, i))}</g>
      <g className="vg-mid">{GATES.slice(1, 3).map((g, i) => torii(g, i))}</g>
      <g className="vg-near">
        {torii(GATES[0], 0)}
        {/* path */}
  ...
  return (
    <Stage label="A bowing deer beside a stone lantern in Nara park">
      ...
      <g className="vg-mid">
        ...
        {/* grass strokes */}
        <g className="vg-grass" stroke="var(--pine)" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.7">
...
function OsakaVignette() {
  return (
    <Stage label="Osaka castle and the giant Dōtonbori crab in neon">
      ...
      <g className="vg-mid">
        <circle className="vg-neon" cx="196" cy="30" r="12" fill="var(--art-blossom-b)" />
        <circle className="vg-neon" cx="170" cy="52" r="7" fill="var(--art-lantern)" />
        <circle className="vg-neon" cx="222" cy="56" r="5" fill="var(--art-blossom-a)" />
      </g>
      {/* the crab */}
      <g className="vg-near">
        ...
        <circle className="vg-blink" cx="284" cy="36" r="4.5" fill="var(--art-silhouette)" />
        <circle className="vg-blink" cx="316" cy="36" r="4.5" fill="var(--art-silhouette)" />
        {/* claws — the right one waves */}
        <g fill="var(--vermillion)">
          <path d="M254 52 q-18 -4 -22 -20 q14 2 22 10 z" />
          <circle cx="256" cy="56" r="10" />
        </g>
        <g className="vg-claw" fill="var(--vermillion)">
  ...
  return (
    <Stage label="A plane climbing over the Pacific toward home">
      ...
      <g className="vg-mid">
        ...
        <g className="vg-waves" stroke="var(--art-silhouette)" strokeWidth="1.6" fill="none" opacity="0.35" strokeLinecap="round">
      ...
      {/* plane climbing right — inked in --ink so it reads day and night.
          Outer g keeps its attribute transform; the ride animates the inner g. */}
      <g className="vg-near">
        <g transform="translate(230 46) rotate(-12)">
          <g className="vg-plane-ride">
            <g fill="var(--ink)">
